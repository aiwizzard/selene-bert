...
class MultiHeadAttention(nn.Module):
    def __init__(self, model_dim=512, head=8, dropout_rate=0.1):
        ...
        self.query_weights = nn.Linear(model_dim, model_dim, False)
        self.key_weights = nn.Linear(model_dim, model_dim, False)
        self.value_weights = nn.Linear(model_dim, model_dim, False)
...
class ScaledDotProductAttention(nn.Module):
    """
    Scaled dot product attention. The input consist of queries and keys of dimension k_dim,
    and values of dimension v_dim.
    """
